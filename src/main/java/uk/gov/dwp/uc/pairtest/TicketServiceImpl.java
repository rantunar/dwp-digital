...
public class TicketServiceImpl implements TicketService {
  ...
  public void purchaseTickets(Long accountId, TicketTypeRequest... ticketTypeRequests)
    ...
    if (ticketTypeRequests == null)
      throw new InvalidPurchaseException(
          ErrorCodes.ERROR05,
          String.format("Purchase data is null for Account id = [%s]", accountId));
    ...
    try {
      int totalAmountToPay =
          ticketTypeRequestList.stream()
              .map(
                  item ->
                      TicketPrices.valueOf(item.getTicketType().name()).getPrice()
                          * item.getNoOfTickets())
  ...
  private boolean isAdultTicketPresent(List<TicketTypeRequest> ticketTypeRequests) {
    return ticketTypeRequests.stream()
        .anyMatch(e -> e.getTicketType().equals(TicketTypeRequest.Type.ADULT));
  }
}
